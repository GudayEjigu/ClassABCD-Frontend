...
const UnitAdd = ({ setShowModal }) => {
  const [englishUnitName, setEnglishUnitName] = useState();
  const [amharicUnitName, setAmharicUnitName] = useState();
  ...
  const addUnitMutationSubmitHandler = async (values) => {
    try {
      addUnitMutation.mutate(
        {
          nameEnglish: englishUnitName,
          nameAmharic: amharicUnitName,
        },
        {
          onSuccess: () => {
            setShowModal(false);
          ...
          onError: (err) => {
            console.log({ err });
            toast.error(err?.response?.data?.message ?? "catUnit add failed");
          },
        }
      );
    } catch (err) {
            ...
            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="felx flex-row w-80">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setEnglishUnitName(e.target.value);
                      }}
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        formik.handleChange(e);
                        setAmharicUnitName(e.target.value);
                      }}
                    />
              ...
              <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                ...
                <button
                  ...
                  type="submit"
                >
                  {addUnitMutation.isLoading ? "saving.." : "Save Changes"}
                </button>
              </div>
            </div>
          </form>
